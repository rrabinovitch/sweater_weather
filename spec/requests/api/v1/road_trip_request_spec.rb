...
RSpec.describe 'Road trip request' do
  it 'returns successful response with origin, destination, travel time, and general forecast info' do
    VCR.use_cassette('Denver to Pueblo road trip') do
      ...
      road_trip_params = {origin: "Denver, CO",
        ...
        expect(road_trip_json[:data][:attributes][:travel_time]).to be_a(Float)
        expect(road_trip_json[:data][:attributes][:forecast_temp]).to eq(road_trip.forecast_temp)
        expect(road_trip_json[:data][:attributes][:forecast_temp]).to be_a(Float)
  ...
  it 'returns successful response with another destination and origin' do
    # VCR.use_cassette('Denver to Pueblo road trip') do
      ...
      road_trip_params = {origin: "Los Angeles, CA",
  ...
  it 'does not create a road trip and returns error if destination param not included in request body' do
    user = create(:user)
    expect(user.road_trips.count).to eq(0)
